...
const serviceRequestSchema = new mongoose.Schema({
    ...
    // Conversation thread between customer and mechanic
    conversation: [{
        sender: { 
            type: mongoose.Schema.Types.ObjectId, 
            ref: 'User', 
...
serviceRequestSchema.methods.addMessage = function (message, senderId, senderRole) {
    ...
    // Update status to 'question' if not already in progress or completed
    if (!['in_progress', 'completed', 'cancelled'].includes(this.status)) {
...
serviceRequestSchema.methods.getUnreadCount = function (userId) {
